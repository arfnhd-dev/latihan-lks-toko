...
![Bagan Alur Laravel](bagan.png)

**Penjelasan singkat:**
- **Browser** mengirim HTTP request ke server
- **routes/web.php** menentukan request diarahkan ke controller mana
- **Middleware** mengecek kondisi (sudah login? role admin?) — kalau tidak lolos, request ditolak
- **Controller** menjalankan logika bisnis, memanggil **Model** untuk ambil/simpan data ke **MySQL**
- **FormRequest** memvalidasi data dari form sebelum diproses controller
- **Blade View** merender HTML yang dikirim balik ke browser sebagai response
- **Storage** dipakai untuk menyimpan file upload (gambar produk)
